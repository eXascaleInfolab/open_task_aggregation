...
class EM:
    ...
    def run(self, iterr=20):
        # self.p_z_i_0, self.p_z_i_1, self.p_phi_j = self.initProbabilities()
        # initialization
...
if __name__ == '__main__':
    datafile = '../input/simple_example.csv'
    annotation_file = '../output/aij.csv'
    ld = LoadData(datafile)

    # extract the influencers features
    answers = pd.read_csv(datafile).drop_duplicates(subset=['infl_acc'])
    influencer_x = answers[['follower_nbr', 'followee_nbr', 'tweets_nbr', 'avg_length_tweets']]
